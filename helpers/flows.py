#https://github.com/HEPML-AnomalyDetection/CATHODE/blob/3d4c0c5d4bd940723cb9de486998ed80ad472b71/flows.py#L163
...
class MADEwithRQS(nn.Module):
    ...
    def __init__(self,
        ...
        input_mask = get_mask_RQS(
            num_inputs, num_hidden, num_inputs, mask_type='input')
        hidden_mask = get_mask_RQS(num_hidden, num_hidden, num_inputs)
        self.bound = bound
        if self.bound:
            self.output_multiplier = (3*num_bins + 1)
        else:
            self.output_multiplier = (3*num_bins - 1)
        output_mask = get_mask_RQS(
